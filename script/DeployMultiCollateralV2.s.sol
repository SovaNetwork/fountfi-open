...
import {Conduit as ConduitMultiCollateral} from "../src/conduit/ConduitMultiCollateral.sol";

contract DeployMultiCollateralV2Script is Script {
    ...
    ConduitMultiCollateral public multiConduit;
    ...
    function run() public {
        ...
        // Deploy multi-collateral specific conduit
        deployMultiCollateralConduit();
    ...
    function deployMultiCollateralConduit() internal {
        // Deploy our multi-collateral conduit
        multiConduit = new ConduitMultiCollateral(address(roleManager));
        console.log("MultiCollateral Conduit deployed:", address(multiConduit));
    }
    ...
    function deployMultiCollateralRegistry() internal {
        ...
        multiCollateralRegistry = new MultiCollateralRegistry(
            address(roleManager),
            address(sovaBTC)
        );
    ...
    function deployStrategy(address deployer) internal {
        ...
        strategyImplementation = new SimpleMultiCollateralStrategy(
            address(sovaBTC),
            8,
            address(multiCollateralRegistry),
            MANAGER_1
        );
        ...
        // Deploy the multi-collateral vault (tRWA)
        // IMPORTANT: We need to use a modified version that uses our multi-collateral conduit
        vault = address(new tRWA(
        ...
        // IMPORTANT: Configure the strategy to recognize our multi-collateral conduit
        console.log("NOTE: The vault will use the registry's conduit for regular deposits");
        console.log("      For multi-collateral deposits, tokens are transferred directly");
    }
    ...
    function logDeployedContracts() internal view {
        ...
        console.log("Registry's Conduit:", registry.conduit());
        console.log("Multi-Collateral Conduit:", address(multiConduit));
        ...
        console.log("1. Users should approve tokens to the vault directly for depositCollateral()");
        console.log("2. For regular deposits, users approve to:", registry.conduit());
        console.log("3. Manager can add redemption funds using depositRedemptionFunds()");
        console.log("4. Exchange rates can be updated in MultiCollateralRegistry");
    }
}