...
contract MockStrategyWithRegistry is IStrategy {
    ...
    function balance() external pure returns (uint256) { return 0; }
    function deposit(uint256) external pure returns (bool) { return true; }
    function redeem(address, uint256) external pure returns (bool) { return true; }
    function withdraw(address, uint256) external {}
    function rescueERC20(address, address, uint256) external {}
    function name() external pure returns (string memory) { return "Mock Strategy"; }
...
contract SimpleMockHook is IHook {
    ...
    function onBeforeDeposit(
        ...
        uint256 assets,
        address receiver
    ) external pure returns (HookOutput memory) {
...
contract tRWAMultiCollateralFullCoverageTest is Test {
    ...
    function setUp() public {
        ...
        // Deploy strategy
        strategy = new SimpleMultiCollateralStrategy(
            address(sovaBTC),
            8,
            address(registry),
            manager
