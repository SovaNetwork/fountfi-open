...
contract MockStrategy is IStrategy, IMultiCollateralStrategy {
    ...
    function deposit(uint256 amount) external returns (bool) { return true; }
    function redeem(address to, uint256 amount) external returns (bool) { return true; }
    function withdraw(address to, uint256 amount) external {}
    function rescueERC20(address tokenAddress, address to, uint256 amount) external {}
    function name() external view returns (string memory) { return "Mock Strategy"; }
    function symbol() external view returns (string memory) { return "MOCK"; }
    function setManager(address newManager) external {}
    
    // IMultiCollateralStrategy implementations
    function assetDecimals() external pure returns (uint8) { return 8; }
    function manager() external pure returns (address) { return address(0); }
    function isHeldCollateral(address) external pure returns (bool) { return false; }
    function heldCollateralTokens(uint256) external pure returns (address) { return address(0); }
    function collateralBalances(address) external pure returns (uint256) { return 0; }
    function totalCollateralValue() external pure returns (uint256) { return 0; }
...
contract MockCollateralRegistry is IMultiCollateralRegistry {
    ...
    function convertToSovaBTC(address, uint256) external pure returns (uint256) { return 0; }
    function convertFromSovaBTC(address, uint256) external pure returns (uint256) { return 0; }
    function getCollateralTokenCount() external view returns (uint256) { return collateralTokens.length; }
    function getAllCollateralTokens() external view returns (address[] memory) { return collateralTokens; }
}
...
contract ConduitMultiCollateralTest is Test {
    ...
    function setUp() public {
        ...
        vm.mockCall(
            address(registry),
            abi.encodeWithSignature("isStrategyToken(address)", address(vault)),
            abi.encode(true)
        );
